...
def main(gpu, args, config):
    ...
    # data_args['eval_size'] = args.image_size
    val_dataset = DynamicFARDataset(data_args, args.image_path, mask_path=None,
                                    batch_size=config['batch_size'], augment=False, training=False,
                                    test_mask_path=args.mask_path, world_size=1)
    ...
    with torch.no_grad():
        for items in tqdm(val_loader):
            ...
            for img_num in range(items['image'].shape[0]):
                cv2.imwrite(os.path.join(eval_path, items['name'][img_num].replace(".jpg", ".png")), gen_img[img_num, :, :, ::-1])
...
if __name__ == '__main__':
    ...
    args.add_argument('--mask_path', default=None, type=str)
